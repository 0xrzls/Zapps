import { Star, Vote, Globe, Twitter, Github } from "lucide-react";
...
const FeaturedDAppsCarousel = () => {
  ...
  useEffect(() => {
    const fetchFeaturedDApps = async () => {
      ...
      try {
        ...
        const result = await backend.dapps.getFeatured(5);
          ...
          for (const id of ids) {
            ...
            try {
              ...
              if (scoreData?.vote_score !== null && scoreData?.vote_score !== undefined) {
                const v = Number(scoreData.vote_score || 0);
                scores[id] = v > 5 ? v / 2 : v;
              }
            } catch (error) {
  ...
  useEffect(() => {
    if (!mobileScrollRef.current || featuredDApps.length === 0) return;

    let currentIndex = 0;
    const interval = setInterval(() => {
      if (mobileScrollRef.current) {
        currentIndex = (currentIndex + 1) % featuredDApps.length;
  ...
  useEffect(() => {
    if (!desktopScrollRef.current || featuredDApps.length === 0) return;
    ...
    const interval = setInterval(() => {
      if (desktopScrollRef.current) {
        ...
        const itemWidth = scrollWidth / featuredDApps.length;
              ...
              <div className="game-card rounded-2xl border border-accent/20 bg-gradient-to-br from-accent/10 via-accent/5 to-transparent relative backdrop-blur-sm px-3 py-3 card-shadow hover:glow-effect transition-smooth cursor-pointer hover:scale-[1.02] before:absolute before:inset-0 before:rounded-2xl before:bg-gradient-to-br before:from-primary/3 before:via-transparent before:to-primary/5 before:pointer-events-none">
                <div className="flex items-center space-x-2.5">
                  ...
                  <div className="flex-1 min-w-0 flex flex-col justify-between py-1">
                    <div className="flex items-center justify-between mb-1">
                      <h3 className="font-medium text-sm leading-tight text-foreground truncate">{dapp.name}</h3>
                      <div className="flex items-center gap-0.5 px-1.5 py-0.5 rounded-full bg-primary/10 border border-primary/20 ml-2 flex-shrink-0">
